...
First of all, you need to create an [Incoming Slack Webhook](https://api.slack.com/incoming-webhooks). Once you have gotten the webhook (in the form https://hooks.slack.com/.../), activate the repository in Drone.

![img](https://i.imgur.com/VBvY5UB.png)

now select the repository and create a Drone secret called `plugin_webhook` in Drone's UI or from the Drone CLI, containing the webhook you just got.

Finally, add [the .drone.yml file](test/.drone.yml) to the repository.

The configuration is extremely simple and relies on a [Docker image](https://hub.docker.com/r/hseeberger/scala-sbt/) containing Scala and SBT.
...
We will use the following [pipeline](build/.drone.yml), which relies on four secrets:
* two secrets to access Nexus, `docker_username` and `docker_password`, which contain Nexus' username and password.
* two secrets to deploy on Kubernetes, `plugin_kubernetes_server` and `plugin_kubernetes_token`, which contain the server and the token needed to access the server.

These secrets can either be created from Drone's web UI (`repo_name -> secrets`) or from the Drone CLI.
